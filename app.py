"""
Dawahi Travel and Tourism Agency — Flask server + real SQLite database
Runs entirely locally on the phone via Termux, with no external internet connection.
...
app.secret_key = "change-this-key-later-dawahi-secret"  # change this before real deployment
app.config["MAX_CONTENT_LENGTH"] = 8 * 1024 * 1024  # maximum 8MB for passport image
...
def init_db():
    ...
    db.executescript(
        ...
        CREATE TABLE IF NOT EXISTS bookings (
            ...
            kind TEXT NOT NULL,               -- 'flight' or 'hotel'
            title TEXT NOT NULL,
            details TEXT,
...
def login_required(view):
    ...
    def wrapped(*args, **kwargs):
        if not current_user():
            return jsonify({"ok": False, "error": "You must be logged in first"}), 401
...
# ---------------------------------------------------------------------------
# HTML pages (same original files without any naming changes)
# ---------------------------------------------------------------------------
@app.route("/")
def root():
...
def api_signup():
    ...
    if expiry_d < date.today():
        return jsonify({"ok": False, "error": "Passport expired! Please use a valid passport."}), 400
    if issue_d > expiry_d:
        return jsonify({"ok": False, "error": "Issue date must be before expiry date"}), 400
...
def api_book():
    ...
    if kind not in ("flight", "hotel") or not title or price <= 0:
        return jsonify({"ok": False, "error": "Invalid booking data"}), 400
    ...
    db.execute(
        """INSERT INTO bookings (code, user_id, kind, title, details, price, status, created_at)
           VALUES (?,?,?,?,?,?,?,?)""",
        (code, user["id"], kind, title, details, price, "confirmed", datetime.now().isoformat()),
    )
    db.commit()
...
if __name__ == "__main__":
    ...
    # When running via gunicorn (on hosting like Render/Railway), the database must be initialized
    # here because the code inside if __name__ == "__main__" won't be executed.
    init_db()